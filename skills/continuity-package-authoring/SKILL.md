...
This is the most important step and the one most often skipped. Also separate **planning from execution**: when the user asks for a plan before proceeding, create a timestamped plan under `.hermes/plans/`, mark the planned sequences explicitly, and wait for/record the user's execution authorization before mutating project artifacts. If authorization is given in the same turn, execute the sequences in order and update the task tracker at each sequence boundary. For every agentic artifact the package describes as existing or functional, check:
