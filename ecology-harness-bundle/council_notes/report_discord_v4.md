...
### One day, eight phases, and a discipline recovered from a 2nd-century astrologer
...
logs, structurally, not promised in prose. Eight phases in a day, judged by
other agents, and the strongest artifact to come out of it is a log of the
campaign's own errors.

## Where it runs

A home lab. A personal, self-hosted Hermes setup on my own machine, built for
my own use: my learning, my motivation, my agency. It was never meant to be a
workplace system and it isn't one — no deployment story, no org integration,
no enterprise anything. Just me, my machines, and a set of rules about who
gets to decide what.
...
and a way of proving they don't. Anyone can write "we value human autonomy"
into a system prompt. Making it machine-checkable is another thing entirely —
and that gap between stated and enforced is where most of the interesting
problems live.
...
corpus, and the discipline eventually stopped being about astrology and
became a way of seeing — because stripped of the astrology, it is exactly the
discipline you want in an agent that must support a human's agency rather
than erode it. So I ran the campaign on it: ten recovered Valens principles,
transposed into machine-checkable form.

The principle I kept coming back to: an interface is an edge, not an
identity. Shared names, shared numbers, shared vocabulary never repair a
missing connection — and in agent systems, the same failure shows up as the
difference between a system that says it respects your judgment and one that
provably does.
...
There's a truth in that log worth sitting with: a system that can't admit its
own errors can't be trusted to protect yours. And admitting errors is
structurally easier than admitting them socially — the log doesn't have to
save face, it just has to be append-only. That's the trick the whole
architecture runs on: move the honesty out of the persona and into the
machinery.

Fun fact, and I mean it as one: the first draft of this report said "all
machine-verified." Row 1 of the calibration log is about a README that did
exactly that. I fixed the report. The discipline is hard even when you're the
one enforcing it — which is the whole point of making it structural rather
than aspirational.
...
guarded. I'd rather you read the raw verdicts and find the 23rd error than
take the 4.7 on faith.
...
calibration log is the evidence either way. That question is the real
research contribution here, and I don't have a clean answer. I have a method
that makes the question answerable, which is the next best thing.
...
I say this for a reason: the campaign's own discipline demands that a
declared state never be mistaken for a working one. The open items are not
footnotes; they're the honest boundary of what a day's work can claim.
