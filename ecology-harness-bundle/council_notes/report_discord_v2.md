...
· Proof-of-concept package: GitHub_PoC/ (accompanies this report)*

---

## The one-sentence version

We spent today building a governed agent architecture on Hermes whose central
rule is: the agent proposes and the human disposes — enforced in verifiers and
logs, structurally, not promised in prose. Eight phases, one day,
independently judged; the strongest artifact is a log of the campaign's own
errors.

## Where it runs (one line, so it's never assumed)

A home lab — a personal, self-hosted Hermes setup on one's own machine, built
for personal use: your own learning, motivation, and agency. It is not a
workplace or enterprise system — no deployment story, no org integration, no
shared infrastructure. It was never meant to be work.
...
coherence, treat worked examples as test vectors. Stripped of the astrology,
that is exactly the discipline you want in an agent that must support a
human's agency rather than erode it — so we ran the campaign on it,
transposing ten recovered Valens principles into machine-checkable form.
...
1. **Foundation** — 8 behavioral-change frameworks (COM-B,
   self-determination theory, motivational interviewing, …) distilled into a
   theory matrix with every link evidence-flagged.
2. **Evidence base** — 70+ sources fetched and checked against live journal
   ...
   papers exiled to a register; opinion articles and preprints honestly
   flagged.
...
rewritten, guarded by itself). A system that can't admit its own errors can't
be trusted to protect yours.

Fun fact: the first draft of this report said "all machine-verified." Row 1
of the calibration log is about a README that did exactly that. We fixed the
report. The discipline is hard even when you're the one enforcing it — which
is the whole point of making it structural.
...
The architecture lived as documentation and verifiers all day; we ran it
once, at the end, as the acceptance test: a live dojo session (an ambivalent
...
Then the user approved it (the author, in this case — stated plainly) — and
that approval is in the record, verbatim. The system refused to arbitrate for
the human even in its own acceptance test.
...
## The loop we'd rather you attack
...
non-circular? We think partly — by externalizing the guards — and the
calibration log is the evidence either way. We'd rather you find the 23rd
